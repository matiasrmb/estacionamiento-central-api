...
from app.db.schema_inventory import (
    ...
    lavados_contract,
    NOCHES_CONFIG_DEFAULTS,
    noches_contract,
    pagos_mensuales_contract,
...
MIGRATION_011_ID = "011_manage_noches_contract"
MIGRATION_012_ID = "012_manage_mensualidades_contract"
MIGRATION_013_ID = "013_manage_gastos_cierres_banos_contract"
MIGRATION_014_ID = "014_manage_gastos_auditoria"
MANAGED_MIGRATION_IDS = (MIGRATION_001_ID, MIGRATION_002_ID, MIGRATION_003_ID, MIGRATION_004_ID, MIGRATION_005_ID, MIGRATION_006_ID, MIGRATION_007_ID, MIGRATION_008_ID, MIGRATION_009_ID, MIGRATION_010_ID, MIGRATION_011_ID, MIGRATION_012_ID, MIGRATION_013_ID, MIGRATION_014_ID)
...
MIGRATIONS: tuple[MigrationMetadata, ...] = (
    ...
    MigrationMetadata(
        MIGRATION_011_ID,
        "Manage the Noches contract without reusing the historical unmanaged 006 SQL file.",
        (),
    ),
    MigrationMetadata(
        MIGRATION_012_ID,
        "Manage the Mensualidades contract without reusing historical unmanaged SQL files.",
        (),
    ),
    MigrationMetadata(
        MIGRATION_013_ID,
        "Manage gastos, cierres, and bathroom-use contracts without historical SQL authority.",
        (),
    ),
    MigrationMetadata(
        MIGRATION_014_ID,
        "Create the operational-expense audit table for admin edit/delete traceability.",
        (
            "CREATE TABLE gastos_operacion_auditoria (\n"
...
def plan_schema_migrations(inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    statuses = {
        ...
        MIGRATION_011_ID: _migration_011_status(inventory, table_names, complete),
        MIGRATION_012_ID: _migration_012_status(inventory, table_names, complete),
        MIGRATION_013_ID: _migration_013_status(inventory, table_names, complete),
        MIGRATION_014_ID: _migration_014_status(inventory, table_names, complete),
    }
    migrations = [
        {
            ...
            "sql": _planned_006_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_006_ID else _planned_007_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_007_ID else _planned_008_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_008_ID else _planned_009_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_009_ID else _planned_010_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_010_ID else _planned_011_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_011_ID else _planned_012_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_012_ID else _planned_013_sql(inventory, statuses[migration.migration_id]) if migration.migration_id == MIGRATION_013_ID else _planned_sql(migration, statuses[migration.migration_id], table_names),
            "will_execute": False,
        }
    ...
    return {
        ...
        "noches_contract": noches_contract(inventory),
        "mensualidades_contract": pagos_mensuales_contract(inventory),
        "gastos_cierres_banos_contract": gastos_cierres_banos_contract(inventory),
...
def apply_009_add_cierres_solo_lavado_totals(engine: Any, **kwargs: Any) -> dict[str, Any]:
    """Apply only additive solo-lavado totals to the existing daily-close table."""
...
def apply_011_manage_noches_contract(engine: Any, **kwargs: Any) -> dict[str, Any]:
    """Apply only the managed Noches contract after explicit confirmation."""
    return _apply(engine, MIGRATION_011_ID, **kwargs)


def apply_012_manage_mensualidades_contract(engine: Any, **kwargs: Any) -> dict[str, Any]:
    """Apply only the managed Mensualidades contract after explicit confirmation."""
    return _apply(engine, MIGRATION_012_ID, **kwargs)


def apply_013_manage_gastos_cierres_banos_contract(engine: Any, **kwargs: Any) -> dict[str, Any]:
    """Apply only the managed gastos, cierres, and bathroom-use contract."""
    return _apply(engine, MIGRATION_013_ID, **kwargs)


def apply_014_manage_gastos_auditoria(engine: Any, **kwargs: Any) -> dict[str, Any]:
    """Apply only the managed operational-expense audit table."""
    return _apply(engine, MIGRATION_014_ID, **kwargs)
...
def _apply(
    ...
    if migration_id == MIGRATION_011_ID:
        return _apply_011(engine, migration["status"], migration["sql"], preflight)
    if migration_id == MIGRATION_012_ID:
        return _apply_012(engine, migration["status"], migration["sql"], preflight)
    if migration_id == MIGRATION_013_ID:
        return _apply_013(engine, migration["status"], migration["sql"], preflight)
    if migration_id == MIGRATION_014_ID:
        return _apply_014(engine, migration["status"], migration["sql"], preflight)
...
def _apply_009(engine: Any, status: str, statements: list[str], preflight: dict[str, Any]) -> dict[str, Any]:
    if status == "repair_required":
        return _record(engine, MIGRATION_009_ID, "repaired", [], preflight)
    if status != "pending":
        return _result(MIGRATION_009_ID, "refused", [], ["Migration is not pending; no SQL was executed."], preflight)
    executed = []
    try:
        with engine.begin() as conn:
            conn.execute(text(statements[0]))
            executed.append("ALTER TABLE")
            conn.execute(text(MIGRATION_RECORD_SQL), {"migration_id": MIGRATION_009_ID})
            executed.append("INSERT migration record")
    except Exception:
...
def _apply_011(engine: Any, status: str, statements: list[str], preflight: dict[str, Any]) -> dict[str, Any]:
    if status == "repair_required":
        return _record(engine, MIGRATION_011_ID, "repaired", [], preflight)
    if status != "pending":
        return _result(MIGRATION_011_ID, "refused", [], ["Migration is not pending; no SQL was executed."], preflight)
    executed = []
    try:
        with engine.begin() as conn:
            for statement in statements[:-1]:
                conn.execute(text(statement))
                executed.append(_statement_type(statement))
            conn.execute(text(MIGRATION_RECORD_SQL), {"migration_id": MIGRATION_011_ID})
            executed.append("INSERT migration record")
    except Exception:
        return _result(MIGRATION_011_ID, "failed_after_migration" if executed else "failed", executed, ["Migration 011 failed; retry only after a fresh inventory validates the full contract."], preflight)
    return _result(MIGRATION_011_ID, "applied", executed, [], preflight)


def _apply_012(engine: Any, status: str, statements: list[str], preflight: dict[str, Any]) -> dict[str, Any]:
    if status == "repair_required":
        return _record(engine, MIGRATION_012_ID, "repaired", [], preflight)
    if status != "pending":
        return _result(MIGRATION_012_ID, "refused", [], ["Migration is not pending; no SQL was executed."], preflight)
    executed = []
    try:
        with engine.begin() as conn:
            for statement in statements[:-1]:
                conn.execute(text(statement))
                executed.append(_statement_type(statement))
            conn.execute(text(MIGRATION_RECORD_SQL), {"migration_id": MIGRATION_012_ID})
            executed.append("INSERT migration record")
    except Exception:
        return _result(MIGRATION_012_ID, "failed_after_migration" if executed else "failed", executed, ["Migration 012 failed; retry only after a fresh inventory validates the full contract."], preflight)
    return _result(MIGRATION_012_ID, "applied", executed, [], preflight)


def _apply_013(engine: Any, status: str, statements: list[str], preflight: dict[str, Any]) -> dict[str, Any]:
    if status == "repair_required":
        return _record(engine, MIGRATION_013_ID, "repaired", [], preflight)
    if status != "pending":
        return _result(MIGRATION_013_ID, "refused", [], ["Migration is not pending; no SQL was executed."], preflight)
    executed = []
    try:
        with engine.begin() as conn:
            for statement in statements[:-1]:
                conn.execute(text(statement))
                executed.append(_statement_type(statement))
            conn.execute(text(MIGRATION_RECORD_SQL), {"migration_id": MIGRATION_013_ID})
            executed.append("INSERT migration record")
    except Exception:
        return _result(MIGRATION_013_ID, "failed_after_migration" if executed else "failed", executed, ["Migration 013 failed; retry only after a fresh inventory validates the full contract."], preflight)
    return _result(MIGRATION_013_ID, "applied", executed, [], preflight)


def _apply_014(engine: Any, status: str, statements: list[str], preflight: dict[str, Any]) -> dict[str, Any]:
    if status == "repair_required":
        return _record(engine, MIGRATION_014_ID, "repaired", [], preflight)
    if status != "pending":
        return _result(MIGRATION_014_ID, "refused", [], ["Migration is not pending; no SQL was executed."], preflight)
    executed = []
    try:
        with engine.begin() as conn:
            for statement in statements[:-1]:
                conn.execute(text(statement))
                executed.append(_statement_type(statement))
            conn.execute(text(MIGRATION_RECORD_SQL), {"migration_id": MIGRATION_014_ID})
            executed.append("INSERT migration record")
    except Exception:
        return _result(MIGRATION_014_ID, "failed_after_migration" if executed else "failed", executed, ["Migration 014 failed; retry only after a fresh inventory validates the audit table."], preflight)
    return _result(MIGRATION_014_ID, "applied", executed, [], preflight)
...
def _migration_009_status(inventory: dict[str, Any], tables: set[str] | None, complete: bool) -> str:
    if not complete:
        return "unknown"
    if any(_migration_recorded(inventory, migration_id) is not True for migration_id in MANAGED_MIGRATION_IDS[:8]):
        return "blocked_prerequisite"
    contract = cierres_solo_lavado_totals_contract(inventory)
    recorded = _migration_recorded(inventory, MIGRATION_009_ID)
    if recorded is True:
...
def _migration_011_status(inventory: dict[str, Any], tables: set[str] | None, complete: bool) -> str:
    if not complete:
        return "unknown"
    if any(_migration_recorded(inventory, migration_id) is not True for migration_id in MANAGED_MIGRATION_IDS[:10]):
        return "blocked_prerequisite"
    contract = noches_contract(inventory)
    recorded = _migration_recorded(inventory, MIGRATION_011_ID)
    if recorded is True:
        return "applied" if contract["valid"] is True else "inconsistent_state"
    if contract["valid"] is True:
        return "repair_required"
    if contract["state"] == "blocked_prerequisite" or any("table is missing" in issue for issue in contract["issues"]):
        return "blocked_prerequisite"
    if contract["create_safe"] or contract["add_safe"]:
        return "pending"
    return "invalid_contract"


def _migration_012_status(inventory: dict[str, Any], tables: set[str] | None, complete: bool) -> str:
    if not complete:
        return "unknown"
    if any(_migration_recorded(inventory, migration_id) is not True for migration_id in MANAGED_MIGRATION_IDS[:11]):
        return "blocked_prerequisite"
    contract = pagos_mensuales_contract(inventory)
    recorded = _migration_recorded(inventory, MIGRATION_012_ID)
    if recorded is True:
        return "applied" if contract["valid"] is True else "inconsistent_state"
    if contract["valid"] is True:
        return "repair_required"
...
def _migration_013_status(inventory: dict[str, Any], tables: set[str] | None, complete: bool) -> str:
    if not complete:
        return "unknown"
    if any(_migration_recorded(inventory, migration_id) is not True for migration_id in MANAGED_MIGRATION_IDS[:12]):
        return "blocked_prerequisite"
    contract = gastos_cierres_banos_contract(inventory)
    recorded = _migration_recorded(inventory, MIGRATION_013_ID)
    if recorded is True:
        return "applied" if contract["valid"] is True else "inconsistent_state"
    if contract["valid"] is True:
        return "repair_required"
    if contract["state"] == "safe_to_add":
        return "pending"
    if contract["state"] == "blocked_orphans":
        return "blocked_prerequisite"
    return "blocked_prerequisite" if contract["state"] == "blocked_prerequisite" else "invalid_contract"


def _migration_014_status(inventory: dict[str, Any], tables: set[str] | None, complete: bool) -> str:
    if not complete:
        return "unknown"
    if any(_migration_recorded(inventory, migration_id) is not True for migration_id in MANAGED_MIGRATION_IDS[:13]):
        return "blocked_prerequisite"
    recorded = _migration_recorded(inventory, MIGRATION_014_ID)
    valid = _gastos_auditoria_contract_valid(inventory, tables)
    if recorded is True:
        return "applied" if valid else "inconsistent_state"
    if valid:
        return "repair_required"
    return "pending" if "gastos_operacion_auditoria" not in tables else "invalid_contract"
...
def _gastos_auditoria_contract_valid(inventory: dict[str, Any], tables: set[str] | None) -> bool:
    if tables is None or "gastos_operacion_auditoria" not in tables:
        return False
    columns = {
        str(row.get("column_name", "")).casefold()
        for row in inventory.get("columns", [])
        if isinstance(row, dict) and str(row.get("table_name", "")).casefold() == "gastos_operacion_auditoria"
    }
    required = {
        "id_auditoria", "id_gasto", "accion", "usuario", "fecha_hora",
...
def _planned_sql(migration: MigrationMetadata, status: str, tables: set[str] | None) -> list[str]:
    ...
    if migration.migration_id in {MIGRATION_003_ID, MIGRATION_004_ID, MIGRATION_005_ID, MIGRATION_014_ID}:
...
def _planned_009_sql(inventory: dict[str, Any], status: str) -> list[str]:
    if status == "repair_required":
        return [MIGRATION_RECORD_SQL]
    if status != "pending":
        return []
    column_sql = {
        "total_lavados_solos": "ADD COLUMN total_lavados_solos INT NOT NULL DEFAULT 0",
        "total_lavados_solos_monto": "ADD COLUMN total_lavados_solos_monto INT NOT NULL DEFAULT 0",
        "total_general": "ADD COLUMN total_general INT NOT NULL DEFAULT 0",
    }
    missing = cierres_solo_lavado_totals_contract(inventory)["missing_columns"]
...
def _planned_011_sql(inventory: dict[str, Any], status: str) -> list[str]:
    if status == "repair_required":
        return [MIGRATION_RECORD_SQL]
    if status != "pending":
        return []
    contract = noches_contract(inventory)
    statements = []
    if contract["missing_cierre_totals"]:
        clauses = {"total_noches": "ADD COLUMN total_noches INT NOT NULL DEFAULT 0", "total_noches_monto": "ADD COLUMN total_noches_monto INT NOT NULL DEFAULT 0"}
        statements.append("ALTER TABLE cierres_diarios\n    " + ",\n    ".join(clauses[name] for name in contract["missing_cierre_totals"]))
    if contract["create_safe"]:
        statements.append("CREATE TABLE cobros_noches (\n    id_cobro_noche INT AUTO_INCREMENT PRIMARY KEY,\n    id_ingreso INT NOT NULL,\n    monto_snapshot INT NOT NULL,\n    hora_inicio_snapshot TIME NOT NULL,\n    hora_fin_snapshot TIME NOT NULL,\n    fecha_hora_pago DATETIME NOT NULL,\n    usuario VARCHAR(50) NOT NULL,\n    estado ENUM('PAGADO', 'ANULADO') NOT NULL DEFAULT 'PAGADO',\n    estado_operativo ENUM('PENDIENTE', 'RETIRADO', 'CONVERTIDO') NOT NULL DEFAULT 'PENDIENTE',\n    fecha_hora_resolucion DATETIME NULL,\n    id_cierre INT NULL,\n    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,\n    INDEX idx_cobros_noches_ingreso (id_ingreso),\n    INDEX idx_cobros_noches_pendiente_cierre (id_cierre, fecha_hora_pago),\n    INDEX idx_cobros_noches_estado_operativo (estado_operativo, id_ingreso),\n    CONSTRAINT fk_cobros_noches_ingreso FOREIGN KEY (id_ingreso) REFERENCES ingresos (id_ingreso),\n    CONSTRAINT fk_cobros_noches_cierre FOREIGN KEY (id_cierre) REFERENCES cierres_diarios (id_cierre)\n)")
    elif contract["add_safe"] and (contract["missing_columns"] or contract["missing_indexes"]):
        columns = {"estado_operativo": "ADD COLUMN estado_operativo ENUM('PENDIENTE', 'RETIRADO', 'CONVERTIDO') NOT NULL DEFAULT 'PENDIENTE'", "fecha_hora_resolucion": "ADD COLUMN fecha_hora_resolucion DATETIME NULL"}
        indexes = {"idx_cobros_noches_estado_operativo": "ADD INDEX idx_cobros_noches_estado_operativo (estado_operativo, id_ingreso)"}
        statements.append("ALTER TABLE cobros_noches\n    " + ",\n    ".join([*(columns[name] for name in contract["missing_columns"]), *(indexes[name] for name in contract["missing_indexes"])]))
    for key in contract["missing_config_keys"]:
        statements.append(f"INSERT INTO configuracion (clave, valor) VALUES ('{key}', '{NOCHES_CONFIG_DEFAULTS[key]}')")
    return [*statements, MIGRATION_RECORD_SQL]


def _planned_012_sql(inventory: dict[str, Any], status: str) -> list[str]:
    if status == "repair_required":
        return [MIGRATION_RECORD_SQL]
    if status != "pending":
        return []
    ...
    statements = []
    vehicle_columns = {
        "dia_vencimiento": "ADD COLUMN dia_vencimiento TINYINT UNSIGNED NOT NULL DEFAULT 1",
        "telefono": "ADD COLUMN telefono VARCHAR(30) NULL DEFAULT NULL",
    }
    cierre_columns = {
        "total_mensualidades": "ADD COLUMN total_mensualidades INT NOT NULL DEFAULT 0",
        "total_mensualidades_monto": "ADD COLUMN total_mensualidades_monto INT NOT NULL DEFAULT 0",
    }
    if contract["missing_vehiculos_columns"]:
        statements.append("ALTER TABLE vehiculos\n    " + ",\n    ".join(vehicle_columns[name] for name in contract["missing_vehiculos_columns"]))
    if contract["missing_cierres_columns"]:
        statements.append("ALTER TABLE cierres_diarios\n    " + ",\n    ".join(cierre_columns[name] for name in contract["missing_cierres_columns"]))
    if contract["create_safe"]:
        statements.append("CREATE TABLE pagos_mensuales (\n    id_pago_mensual INT AUTO_INCREMENT PRIMARY KEY,\n    id_vehiculo INT NOT NULL,\n    periodo DATE NOT NULL,\n    dia_vencimiento_snapshot TINYINT UNSIGNED NOT NULL,\n    monto_snapshot INT NOT NULL,\n    fecha_pago DATETIME NOT NULL,\n    usuario VARCHAR(50) NOT NULL,\n    metodo_pago VARCHAR(50) NULL,\n    observacion VARCHAR(500) NULL,\n    id_cierre INT NULL,\n    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,\n    CONSTRAINT uq_pagos_mensuales_vehiculo_periodo UNIQUE (id_vehiculo, periodo),\n    INDEX idx_pagos_mensuales_pendiente_cierre (id_cierre, fecha_pago),\n    INDEX idx_pagos_mensuales_periodo (periodo),\n    CONSTRAINT fk_pagos_mensuales_vehiculo FOREIGN KEY (id_vehiculo) REFERENCES vehiculos (id_vehiculo),\n    CONSTRAINT fk_pagos_mensuales_cierre FOREIGN KEY (id_cierre) REFERENCES cierres_diarios (id_cierre)\n) ENGINE=InnoDB")
    elif contract["add_safe"]:
        columns = {
            "id_pago_mensual": "ADD COLUMN id_pago_mensual INT AUTO_INCREMENT PRIMARY KEY", "id_vehiculo": "ADD COLUMN id_vehiculo INT NOT NULL",
            "periodo": "ADD COLUMN periodo DATE NOT NULL", "dia_vencimiento_snapshot": "ADD COLUMN dia_vencimiento_snapshot TINYINT UNSIGNED NOT NULL",
            "monto_snapshot": "ADD COLUMN monto_snapshot INT NOT NULL", "fecha_pago": "ADD COLUMN fecha_pago DATETIME NOT NULL",
            "usuario": "ADD COLUMN usuario VARCHAR(50) NOT NULL", "metodo_pago": "ADD COLUMN metodo_pago VARCHAR(50) NULL",
            "observacion": "ADD COLUMN observacion VARCHAR(500) NULL", "id_cierre": "ADD COLUMN id_cierre INT NULL",
            "created_at": "ADD COLUMN created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP",
        }
        indexes = {
            "uq_pagos_mensuales_vehiculo_periodo": "ADD CONSTRAINT uq_pagos_mensuales_vehiculo_periodo UNIQUE (id_vehiculo, periodo)",
            "idx_pagos_mensuales_pendiente_cierre": "ADD INDEX idx_pagos_mensuales_pendiente_cierre (id_cierre, fecha_pago)",
            "idx_pagos_mensuales_periodo": "ADD INDEX idx_pagos_mensuales_periodo (periodo)",
        }
        fks = {
            "vehiculo": "ADD CONSTRAINT fk_pagos_mensuales_vehiculo FOREIGN KEY (id_vehiculo) REFERENCES vehiculos (id_vehiculo)",
            "cierre": "ADD CONSTRAINT fk_pagos_mensuales_cierre FOREIGN KEY (id_cierre) REFERENCES cierres_diarios (id_cierre)",
        }
        clauses = [*(columns[name] for name in contract["missing_columns"]), *(indexes[name] for name in contract["missing_indexes"]), *(fks[name] for name in contract["missing_foreign_keys"])]
        if clauses:
            statements.append("ALTER TABLE pagos_mensuales\n    " + ",\n    ".join(clauses))
    return [*statements, MIGRATION_RECORD_SQL]


def _planned_013_sql(inventory: dict[str, Any], status: str) -> list[str]:
    if status == "repair_required":
        return [MIGRATION_RECORD_SQL]
    if status != "pending":
        return []
    ...
    statements = []
    cierre_columns = {
        "total_gastos": "ADD COLUMN total_gastos INT NOT NULL DEFAULT 0",
        "total_neto": "ADD COLUMN total_neto INT NOT NULL DEFAULT 0",
    }
    if contract["missing_cierre_totals"]:
        statements.append("ALTER TABLE cierres_diarios\n    " + ",\n    ".join(cierre_columns[name] for name in contract["missing_cierre_totals"]))
    child_sql = {
        "gastos_operacion": "CREATE TABLE gastos_operacion (\n    id_gasto INT AUTO_INCREMENT PRIMARY KEY,\n    fecha_hora DATETIME NOT NULL,\n    categoria VARCHAR(80) NOT NULL,\n    descripcion VARCHAR(500) NOT NULL,\n    monto INT NOT NULL,\n    usuario VARCHAR(50) NOT NULL,\n    id_cierre INT NULL,\n    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,\n    INDEX idx_gastos_operacion_pendiente (id_cierre, fecha_hora),\n    CONSTRAINT fk_gastos_operacion_cierre FOREIGN KEY (id_cierre) REFERENCES cierres_diarios (id_cierre)\n) ENGINE=InnoDB",
        "usos_bano": "CREATE TABLE usos_bano (\n    id INT AUTO_INCREMENT PRIMARY KEY,\n    fecha_hora DATETIME NOT NULL,\n    monto INT NOT NULL,\n    usuario VARCHAR(50) NOT NULL,\n    id_cierre INT NULL,\n    INDEX idx_usos_bano_pendiente (id_cierre, fecha_hora),\n    CONSTRAINT fk_usos_bano_cierre FOREIGN KEY (id_cierre) REFERENCES cierres_diarios (id_cierre)\n) ENGINE=InnoDB",
    }
    for table in ("gastos_operacion", "usos_bano"):
        child = contract[table]
        if child["create_safe"]:
            statements.append(child_sql[table])
            continue
        clauses = []
        if child["widen_category"]:
            clauses.append("MODIFY COLUMN categoria VARCHAR(80) NOT NULL")
        if "id_cierre" in child["missing_columns"]:
            clauses.append("ADD COLUMN id_cierre INT NULL")
        index_name = "idx_gastos_operacion_pendiente" if table == "gastos_operacion" else "idx_usos_bano_pendiente"
        if index_name in child["missing_indexes"]:
            clauses.append(f"ADD INDEX {index_name} (id_cierre, fecha_hora)")
        if "cierre" in child["missing_foreign_keys"]:
            clauses.append(f"ADD CONSTRAINT {'fk_gastos_operacion_cierre' if table == 'gastos_operacion' else 'fk_usos_bano_cierre'} FOREIGN KEY (id_cierre) REFERENCES cierres_diarios (id_cierre)")
        if clauses:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--apply-011-manage-noches-contract", action="store_true")
    parser.add_argument("--apply-012-manage-mensualidades-contract", action="store_true")
    parser.add_argument("--apply-013-manage-gastos-cierres-banos-contract", action="store_true")
    parser.add_argument("--apply-014-manage-gastos-auditoria", action="store_true")
    ...
    apply_flags = [args.apply_001_create_schema_migrations, args.apply_002_create_tipos_lavado, args.apply_003_widen_pagos_mensuales_metodo_pago, args.apply_004_add_operaciones_servicio_ingreso_generado_fk, args.apply_005_add_operaciones_servicio_tipo_vehiculo_lavado_fk, args.apply_006_create_lavados_and_ingresos_en_lavado, args.apply_007_migrate_wash_vehicle_type_pricing, args.apply_008_complete_operaciones_servicio_contract, args.apply_009_add_cierres_solo_lavado_totals, args.apply_010_add_asistencias_device_sessions, args.apply_011_manage_noches_contract, args.apply_012_manage_mensualidades_contract, args.apply_013_manage_gastos_cierres_banos_contract, args.apply_014_manage_gastos_auditoria]
    ...
    try:
        ...
        if args.dry_run:
            ...
            apply = (
                ...
                else apply_010_add_asistencias_device_sessions if args.apply_010_add_asistencias_device_sessions
                else apply_011_manage_noches_contract
                if args.apply_011_manage_noches_contract
                else apply_012_manage_mensualidades_contract
                if args.apply_012_manage_mensualidades_contract
                else apply_013_manage_gastos_cierres_banos_contract
                if args.apply_013_manage_gastos_cierres_banos_contract
                else apply_014_manage_gastos_auditoria
            )
            apply_kwargs = {
                "backup_confirmed": args.backup_confirmed,
